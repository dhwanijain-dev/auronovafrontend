...
// Define types for queue data and props
interface QueueData {
  id: number;
  name: string;
  queueLength: number;
...
interface FoodStallProps {
  stall: QueueData;
  setSelectedStall: (stall: QueueData) => void;
}
...
function Person({ position, queueLength }: PersonProps) {
...
function FoodStall({ stall, setSelectedStall }: FoodStallProps) {
...
function CameraController({ stalls }: CameraControllerProps) {
  const { camera } = useThree();
  const longestQueue = useRef<number | null>(null);
...
export default function CafeteriaQueueMap() {
  const [selectedStall, setSelectedStall] = useState<QueueData | null>(null);
