"use client"

import { Card, CardDescription, CardHeader, CardTitle } from "@/components/ui/card"

import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs"
import CafeteriaQueueMap from "@/components/cafeteria-queue-map"
import QueueHeatmap from "@/components/queue-heatmap"
import DynamicUI from "@/components/dynamic-ui"
import GamificationRewards from "@/components/gamification-rewards"
import MicroInteractions from "@/components/micro-interactions"
import AnalyticsDashboard from "@/components/analytics-dashboard"
import { Button } from "@/components/ui/button"
import { Menu } from "lucide-react"
import { useState } from "react"

export default function Home() {
  // const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false)

  return (
    <main className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 text-white p-2 sm:p-4 md:p-6">
      {/* <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6"> */}
          {/* <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-purple-400 to-pink-600">
            Cafeteria Queue Management
          </h1> */}

          {/* <div className="md:hidden mt-4">
            <Button
              variant="outline"
              size="sm"
              onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
              className="w-full flex items-center justify-between"
            >
              <span>Menu</span>
              <Menu className="h-4 w-4" />
            </Button>
          </div>
        </div> */}

        {/* <Tabs defaultValue="dashboard" className="w-full">
          <TabsList
            className={`${isMobileMenuOpen ? "grid" : "hidden"} md:grid grid-cols-1 sm:grid-cols-3 md:grid-cols-6 mb-6 md:mb-8`}
          >
            <TabsTrigger value="dashboard" className="data-[state=active]:bg-purple-700">
              Dashboard
            </TabsTrigger>
            <TabsTrigger value="3d-map" className="data-[state=active]:bg-purple-700">
              3D Queue Map
            </TabsTrigger>
            <TabsTrigger value="heatmap" className="data-[state=active]:bg-purple-700">
              Queue Heatmap
            </TabsTrigger>
            <TabsTrigger value="dynamic-ui" className="data-[state=active]:bg-purple-700">
              Dynamic UI
            </TabsTrigger>
            <TabsTrigger value="gamification" className="data-[state=active]:bg-purple-700">
              Gamification
            </TabsTrigger>
            <TabsTrigger value="micro" className="data-[state=active]:bg-purple-700">
              Micro-Interactions
            </TabsTrigger>
          </TabsList>

          <TabsContent value="dashboard" className="mt-0">
            <AnalyticsDashboard />
          </TabsContent>

          <TabsContent value="3d-map" className="mt-0">
            <CafeteriaQueueMap />
          </TabsContent>

          <TabsContent value="heatmap" className="mt-0">
            <QueueHeatmap />
          </TabsContent>

          <TabsContent value="dynamic-ui" className="mt-0">
            <DynamicUI />
          </TabsContent>

          <TabsContent value="gamification" className="mt-0">
            <GamificationRewards />
          </TabsContent>

          <TabsContent value="micro" className="mt-0">
            <MicroInteractions />
          </TabsContent>
        </Tabs> */}
      {/* </div> */}
    </main>
    // <div className="h-screen w-full bg-[#04080F]">
    //   <Card>
    //     <CardHeader></CardHeader>
    //     <CardTitle>Queue Management System</CardTitle>
    //     <CardDescription>
    //       This is a queue management system for a cafeteria. It allows users to view the current queue status, estimated wait times, and other relevant information.
    //     </CardDescription>
    //     </Card>
    // </div>
  )
}

